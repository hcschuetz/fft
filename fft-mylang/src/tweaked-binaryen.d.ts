// Hacks:
// - While the corresponding JS file just passes through the runtime
//   binaryen API, the type declarations given here are stricter than in the
//   original binaryen code, where many pointer types just collapse to `number`.
//   (We use empty `const enum` types to make the pointer types
//   distinguishable.)
// - Some `const enum`s from the original typing have been converted to plain
//   `enum`s so that they can be used in reflective code converting ids to
//   names.
